...
if __name__ == '__main__':
    ...
    agent_size = 2
    test_data = torch.rand(agent_size, 1, 4, 512, 512, 3)
    test_data = test_data.cuda()

    extrinsic = torch.rand(agent_size, 1, 4, 4, 4)
    intrinsic = torch.rand(agent_size, 1, 4, 3, 3)
    transformation_matrix = torch.from_numpy(np.identity(4))
    transformation_matrix = einops.repeat(transformation_matrix,
                                          'h w -> b l h w', b=1, l=agent_size)
    record_len = torch.from_numpy(np.array([agent_size], dtype=int))
    ...
    params = load_yaml('opencood/hypes_yaml/opcamera/cvt_swap_fuse.yaml')
    params['model']['args']['max_cav'] = agent_size
    params['model']['args']['swap_fusion']['agent_size'] = agent_size
    model = CrossViewTransformerSwapFuse(params['model']['args'])
    model = model.cuda()
